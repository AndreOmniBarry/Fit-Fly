...
async function computeCurrentValues(db) {
    const [sleepLogs, meditationSessions, bpSamples, spo2Samples, stepEntries, hydrationEntries, runs, sessions,] = await Promise.all([
    ...
    const vitalsDates = [...bpSamples, ...spo2Samples].map((s) => s.recordedAt.slice(0, 10));
    const bestStepsDay = bestStepsDayEver(stepEntries);
    const lifetimeSteps = stepEntries.reduce((sum, e) => sum + e.steps, 0);
    const lifetimeHydrationL = sumHydrationEntries(hydrationEntries) / 1000;
    const lifetimeRunKm = runs.reduce((sum, r) => sum + r.distanceMeters, 0) / 1000;
    return {
...
export async function evaluateAllBadges(db = getDb()) {
