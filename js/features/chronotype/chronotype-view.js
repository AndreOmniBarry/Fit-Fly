...
import { CHRONOTYPE_QUESTIONS, scoreChronotype, describeChronotypeCategory, chronotypeCategoryLabel, } from './chronotype.js';
import { recordChronotypeAssessment, listRecentChronotypeAssessments, } from '../../db/repositories/chronotype-assessments.js';
function byId(id) {
    const el = document.getElementById(id);
    if (!el)
        throw new Error(`chronotype-view: missing #${id}`);
    return el;
}
...
function renderQuestions() {
    ...
    container.innerHTML = CHRONOTYPE_QUESTIONS.map((q, index) => `
      <div class="field" style="${index === 0 ? '' : 'margin-top:var(--space-4);'}">
        ...
        <div class="chip-group" id="chronotype-q-${q.id}" role="group" aria-labelledby="lbl-chronotype-${q.id}">
          ${q.options
        .map((opt) => `<button type="button" class="chip" data-value="${opt.points}" aria-pressed="false">${opt.label}</button>`)
        .join('')}
        </div>
      </div>
    `).join('');
    questionChips = new Map(CHRONOTYPE_QUESTIONS.map((q) => [q.id, initChipGroup(byId(`chronotype-q-${q.id}`))]));
}
function resetForm() {
    for (const chips of questionChips.values())
        chips.setValue(null);
...
function openAssessment() {
    const form = byId('chronotype-form');
    if (latest) {
...
export function initChronotypeFeature() {
    renderQuestions();
    ...
    byId('btn-chronotype-retake').addEventListener('click', () => {
        resetForm();
        byId('chronotype-result-card').hidden = true;
        byId('chronotype-form').hidden = false;
    });
    byId('chronotype-form').addEventListener('submit', async (event) => {
        ...
        renderEntryCard();
        renderResultCard(latest);
        byId('chronotype-form').hidden = true;
    });
...
//# sourceMappingURL=chronotype-view.js.map