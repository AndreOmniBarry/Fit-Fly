...
function byId(id) {
    ...
    if (!el)
        throw new Error(`focus-view: missing #${id}`);
    return el;
}
...
export function initFocusFeature() {
    ...
    function render(state) {
        ...
        const volumeInput = byId('focus-volume');
        ...
        for (const pill of timerRow.querySelectorAll('.focus-timer-pill')) {
    ...
    byId('focus-volume').addEventListener('input', (event) => {
        const value = Number(event.target.value);
        engine.setVolume(value / 100);
    });
    timerRow.addEventListener('click', (event) => {
        const button = event.target.closest('.focus-timer-pill');
...
//# sourceMappingURL=focus-view.js.map