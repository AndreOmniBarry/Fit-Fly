...
export function getSavedKokoroVoice(): KokoroVoiceId {
  const saved = getPref(KOKORO_VOICE_PREF_KEY);
  return saved != null && saved in KOKORO_VOICES ? (saved as KokoroVoiceId) : DEFAULT_KOKORO_VOICE;
}

export function setSavedKokoroVoice(voice: KokoroVoiceId): void {
  setPref(KOKORO_VOICE_PREF_KEY, voice);
}

export interface KokoroDownloadProgress {
  /** 0-100, aggregated across every file Kokoro needs (tokenizer, model
   *  weights, the chosen voice's style vector) — a single honest number,
   *  not "file 1 of 4" busywork. */
  percent: number;
  /** The file currently most active, for a real status line ("Loading
   *  model.onnx…") instead of a bare percentage. */
  file: string | null;
}
...
export function breathPauseMs(speed = 1): number {
  const base = 260;
...
interface KokoroTTSInstance {
  generate(text: string, opts: { voice: KokoroVoiceId; speed?: number }): Promise<KokoroRawAudio>;
  stream(
    text: string,
    opts: { voice: KokoroVoiceId; speed?: number }
  ): AsyncGenerator<{ text: string; audio: KokoroRawAudio }>;
}
...
export async function ensureKokoroLoaded(onProgress?: (p: KokoroDownloadProgress) => void): Promise<void> {
  if (ttsPromise) {
    await ttsPromise;
    return;
  }
  ...
  ttsPromise = (async () => {
    try {
      if (!modulePromise) modulePromise = import(KOKORO_ENGINE_URL) as Promise<KokoroModule>;
...
export async function forgetKokoroModel(clearCaches: boolean): Promise<void> {
  modulePromise = null;
  ttsPromise = null;
  loadedInstance = null;
  loadFailed = false;
...
export async function speakWithKokoro(
  text: string,
  {
    ...
    speed = 1,
    onAudioStart,
  }: { voice?: KokoroVoiceId; speed?: number; onAudioStart?: () => void } = {}
): Promise<void> {
  if (!loadedInstance) throw new Error('Kokoro is not loaded yet — call ensureKokoroLoaded() first.');
  ...
  for await (const { audio } of tts.stream(text, { voice, speed })) {
    if (token !== speakToken) return; // superseded by a newer speakWithKokoro()/stopKokoroSpeaking() call
...
async function playBlob(blob: Blob, token: number, onAudioStart?: () => void): Promise<void> {
  ...
  if (ctx.state === 'suspended') {
    try {
      await Promise.race([
        ctx.resume(),
        new Promise<void>((_, reject) => setTimeout(() => reject(new Error('resume timeout')), CONTEXT_RESUME_TIMEOUT_MS)),
      ]);
    } catch {
      return; // never confirmed running — let the caller's own fallback take over
    }
    if ((ctx.state as AudioContextState) !== 'running' || token !== speakToken) return;
  }
