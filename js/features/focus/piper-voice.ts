...
let readySession: TtsSession | null = null;
...
export function didPiperLoadFail(): boolean {
  return loadFailed;
}
...
export async function ensurePiperLoaded(): Promise<void> {
  if (sessionPromise) {
...
export async function speakWithPiper(text: string): Promise<void> {
  if (!readySession) throw new Error('Piper is not loaded yet — call ensurePiperLoaded() first.');
  const session = readySession;
  const token = ++speakToken;
  const blob = await session.predict(text);
  if (token !== speakToken) return; // superseded by a newer speakWithPiper()/stopPiperSpeaking() call
