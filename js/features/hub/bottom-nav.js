...
const TOP_LEVEL_SCREENS = ['screen-hub', 'screen-home', 'screen-badges', 'screen-settings'];
function isTopLevelScreen(id) {
    return TOP_LEVEL_SCREENS.includes(id ?? '');
}
...
function byId(id) {
    ...
    if (!el)
        throw new Error(`bottom-nav: missing #${id}`);
    return el;
}
export function initBottomNav() {
    const nav = byId('bottom-nav');
    const buttons = Array.from(nav.querySelectorAll('.bottom-nav-btn'));
    ...
    function setNavHeightVar(visible) {
