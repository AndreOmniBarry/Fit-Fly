...
import type { Profile } from '../../db/repositories/profile.js';
...
export function initHubStatsFeature(): void {
...
async function refreshHubStats(): Promise<void> {
  ...
  const [profile, allSteps, hydrationEntries, recentSleepLogs] = await Promise.all([
    getProfile(),
    listAllStepEntries() as Promise<StepEntry[]>,
    listHydrationEntriesInRange(startDate, endDate) as Promise<HydrationEntry[]>,
    listRecentSleepLogs(14) as Promise<SleepLog[]>,
  ]);
...
function trailingStartDate(endDate: string, days: number): string {
  const anchor = new Date(`${endDate}T00:00:00`);
  anchor.setDate(anchor.getDate() - (days - 1));
  return anchor.toISOString().slice(0, 10);
}

function renderGreeting(profile: Profile | undefined, now: Date): void {
...
function renderStepsCard(allSteps: StepEntry[], profile: Profile | undefined, endDate: string): void {
  const week = trailingDailyTotals(
    allSteps.map((e) => ({ date: e.date, value: e.steps })),
    { days: TRAILING_DAYS, endDate }
  );
...
function renderWaterCard(hydrationEntries: HydrationEntry[], endDate: string): void {
  const week = trailingDailyTotals(
    hydrationEntries.map((e) => ({ date: e.date, value: e.amountMl })),
    { days: TRAILING_DAYS, endDate }
  );
...
function renderSleepCard(recentSleepLogs: SleepLog[], profile: Profile | undefined, endDate: string): void {
