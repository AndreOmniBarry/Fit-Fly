...
export function trailingDailyTotals(
  ...
  { days, endDate }: { days: number; endDate: string }
): DailyTotal[] {
...
export interface Greeting {
  ...
  /** "Good morning" etc. — real, local-clock-derived, never a canned
   *  rotating slogan pretending to know anything about the user. */
  text: string;
}
...
export function greetingForHour(hour: number): Greeting {
  if (hour < 5) return { period: 'night', text: 'Good night' };
  if (hour < 12) return { period: 'morning', text: 'Good morning' };
...
export function formatHeroDistanceKm(meters: number): string {
