...
export function setHearingTileSubtitle(text) {
...
export function setActiveEnergyText(text) {
...
export function setHubGreeting({ eyebrow, name }) {
...
export function setHeroStepsCard(data) {
...
export function clearHeroStepsChart() {
...
export function setHeroCaloriesCard(data) {
    const group = bySvgId('hub-stat-calories-ring-segments');
    group.innerHTML = '';
...
export function setHeroWaterCard(data) {
    byId('hub-stat-water-value').textContent = `${data.todayMl.toLocaleString()} ml`;
    const svg = bySvgId('hub-stat-water-chart');
    svg.innerHTML = '';
    const width = 120;
    const height = 36;
    const geometry = buildSmoothAreaGeometry(data.week.map((d) => d.value), { width, height });
...
export function setHeroSleepCard(data) {
    ...
    const width = 120;
    const height = 36;
    const geometry = buildSmoothAreaGeometry(data.week.map((d) => d.score), { width, height, floorValue: 0 });
    if (geometry.points.length < 2)
        return;
    const ns = 'http://www.w3.org/2000/svg';
    const defs = document.createElementNS(ns, 'defs');
