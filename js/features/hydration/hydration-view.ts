...
const EXPRESSION_MOUTH_D: Record<'low' | 'mid' | 'full', string> = {
...
function byId<T extends HTMLElement = HTMLElement>(id: string): T {
  ...
  if (!el) throw new Error(`hydration-view: missing #${id}`);
  return el as T;
}

function getGoalMl(): number {
...
export function initHydrationFeature(): void {
  ...
  byId<HTMLInputElement>('hydration-interval-hours').value = String(getReminderIntervalHours());

  // ---------- quick log ----------
  byId('hydration-quick-log').querySelectorAll<HTMLButtonElement>('.hydration-quick-log-btn').forEach((btn) => {
  ...
  byId('btn-hydration-custom-save').addEventListener('click', async () => {
    const amountMl = Number(byId<HTMLInputElement>('hydration-custom-ml').value);
  ...
  byId('btn-hydration-goal-save').addEventListener('click', () => {
    const goal = Number(byId<HTMLInputElement>('hydration-goal-input').value);
    if (!Number.isFinite(goal) || goal < 500 || goal > 6000) return;
    setPref(GOAL_PREF_KEY, String(goal));
    byId<HTMLInputElement>('hydration-goal-input').value = '';
  ...
  byId<HTMLInputElement>('hydration-interval-hours').addEventListener('change', (event) => {
    const hours = Number((event.target as HTMLInputElement).value);
    if (!Number.isFinite(hours) || hours < 1 || hours > 8) return;
...
function renderNotifyStatus(): void {
  ...
  const statusText: Record<NotificationPermissionState, string> = {
...
function renderFigure(todayMl: number): void {
  ...
  const level: 'low' | 'mid' | 'full' = fraction >= 1 ? 'full' : fraction >= 0.4 ? 'mid' : 'low';
...
function renderHistory(todayEntries: HydrationEntry[]): void {
