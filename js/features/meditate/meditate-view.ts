...
import type { GuidedSessionPlayerHandle } from '../focus/guided-session-view.js';
import type { GuidedSession } from './meditations.js';
...
function byId<T extends HTMLElement = HTMLElement>(id: string): T {
  ...
  if (!el) throw new Error(`meditate-view: missing #${id}`);
  return el as T;
}
...
function formatSessionLength(session: GuidedSession): string {
  const minutes = totalDurationSeconds(session) / 60;
  return minutes < 1 ? 'under 1 min' : `${Math.round(minutes)} min`;
}

export function initMeditateFeature(player: GuidedSessionPlayerHandle): void {
  function playSession(session: GuidedSession): void {
  ...
  function buildGrid(grid: HTMLElement, sessions: readonly GuidedSession[]): void {
  ...
  async function refreshStats(): Promise<void> {
    ...
    setMeditateTileSubtitle(
      streak > 0 ? `${streak}-day streak` : 'Guided meditation & breathwork'
    );
  }
