...
function minutesSinceNoon(isoDateTime) {
...
export function calculateSleepConsistency(recentLogs) {
    const bedTimes = recentLogs
        .map((log) => log.bedTime)
        .filter((bedTime) => bedTime != null);
    ...
    const score = Math.round(Math.min(100, Math.max(0, 100 - (stdDev / ZERO_SCORE_STDDEV_MINUTES) * 100)));
    return { score, varianceMinutes: Math.round(stdDev), nightsConsidered: bedTimes.length };
}
//# sourceMappingURL=sleep-consistency.js.map