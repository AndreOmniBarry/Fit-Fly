...
function nightQualityProxy(log, goalMinutes) {
...
export function calculateSleepFactorInsights(logs, goalMinutes = DEFAULT_SLEEP_GOAL_MINUTES) {
    const withBedTime = logs.filter((log) => log.bedTime != null);
    const insights = [];
    if (withBedTime.length >= MIN_NIGHTS_PER_GROUP * 2) {
        ...
        const consistent = compareGroups(withBedTime, goalMinutes, (log) => Math.abs(bedtimeMinutesSinceNoon(log.bedTime) - medianVal) <= CONSISTENT_BEDTIME_WINDOW_MINUTES, 'Consistent bedtime');
        if (consistent)
            insights.push(consistent);
        const lateNights = compareGroups(withBedTime, goalMinutes, (log) => {
        ...
        if (lateNights)
            insights.push(lateNights);
    }
    return insights;
}
//# sourceMappingURL=sleep-insights.js.map