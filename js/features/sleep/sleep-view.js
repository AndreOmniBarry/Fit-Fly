...
export function initSleepFeature() {
    ...
    let sleepInsightRange = 'W';
    let cachedAllSleepLogs = [];
    ...
    async function loadInsightChart() {
    ...
    function scoreNightForChart(sortedLogs, index) {
    ...
    function renderInsightChart() {
        ...
        const insightNights = nightsInRange.map(({ log, index }) => ({
            date: log.date,
        ...
        buckets.forEach((bucket, i) => {
            ...
            btn.append(tooltip);
            const showTooltip = () => {
                for (const other of pointsLayer.querySelectorAll('.trend-chart-tooltip'))
                    other.hidden = true;
    ...
    function renderBestNightBadge(sortedAllLogs) {
    ...
    initChipGroup(byId('sleep-insight-range'), {
