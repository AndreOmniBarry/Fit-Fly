...
export function initSleepFeature(): void {
  ...
  async function loadDashboard(): Promise<void> {
    ...
    if (todayLog) renderResult(todayLog);
    else renderForm();
  }
  ...
  byId('btn-sleep-edit-log').addEventListener('click', () => {
    if (!todayLog) return;
    byId<HTMLInputElement>('sleep-log-bedtime').value = todayLog.bedTime ? formatTimeInputValue(todayLog.bedTime) : '';
    byId<HTMLInputElement>('sleep-log-waketime').value = todayLog.wakeTime ? formatTimeInputValue(todayLog.wakeTime) : '';
    qualityChips.setValue(todayLog.quality == null ? null : String(todayLog.quality));
    byId<HTMLTextAreaElement>('sleep-log-notes').value = todayLog.notes ?? '';
    renderForm();
  });
