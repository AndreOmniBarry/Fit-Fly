...
export interface StepTrailMilestone {
  fraction: number; // 0, 0.25, 0.5, 0.75, 1 — a fifth of the real goal each
  reached: boolean;
}
...
export function buildStepTrailMilestones(steps: number, goal: number): StepTrailMilestone[] {
...
export function stepTrailProgress(steps: number, goal: number): number {
  if (!(goal > 0)) return 0;
