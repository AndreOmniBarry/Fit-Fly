...
export interface StepsDistanceEstimate {
  meters: number;
  method: 'height-stride-formula';
}
...
}): StepsDistanceEstimate | null {
  if (!(steps > 0) || !(heightCm != null && heightCm > 0)) return null;
...
export function formatStepsDistance(meters: number): string {
  if (meters >= 1000) return `~${(meters / 1000).toFixed(1)} km walked today`;
