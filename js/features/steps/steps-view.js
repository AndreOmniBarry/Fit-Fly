...
function renderJourney(steps, heightCm) {
    const goal = getGoal();
    const path = byId('step-trail-path');
    ...
    for (const milestone of buildStepTrailMilestones(steps, goal)) {
        ...
        use.setAttribute('class', milestone.reached ? 'step-trail-milestone step-trail-milestone--reached' : 'step-trail-milestone');
    ...
    if (estimate != null)
        distanceEl.textContent = formatStepsDistance(estimate.meters);
}
