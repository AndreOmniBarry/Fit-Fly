...
export interface LapRecord {
  ...
  lapMs: number;
  /** Cumulative time since Start — the same number a "split" button on a
   *  real stopwatch would record at this same instant. */
  splitMs: number;
}
...
export function buildLapRecord(lapNumber: number, previousSplitMs: number, currentElapsedMs: number): LapRecord {
  return {
    lapNumber,
...
export interface LapHighlights {
...
export function classifyLaps(laps: LapRecord[]): LapHighlights {
  if (laps.length < MIN_LAPS_FOR_HIGHLIGHT) return { fastestLapNumber: null, slowestLapNumber: null };

  let fastest = laps[0] as LapRecord;
  let slowest = laps[0] as LapRecord;
  ...
  if (fastest.lapMs === slowest.lapMs) return { fastestLapNumber: null, slowestLapNumber: null };
