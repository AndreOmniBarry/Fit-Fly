...
function byId(id) {
    ...
    if (!el)
        throw new Error(`stopwatch-view: missing #${id}`);
    return el;
}
export function initStopwatchFeature() {
    ...
    function checkAlert(elapsedMs) {
        if (alertIntervalMinutes <= 0)
            return;
        const wholeMinutes = Math.floor(elapsedMs / 60_000);
        if (wholeMinutes <= 0 || wholeMinutes === lastAlertedMinute || wholeMinutes % alertIntervalMinutes !== 0)
            return;
    ...
    function render() {
        ...
        renderFab(elapsedMs);
        if (stopwatch.running)
            checkAlert(elapsedMs);
    }
    function renderButtons() {
        const s = status();
        const primary = byId('btn-stopwatch-primary');
        const secondary = byId('btn-stopwatch-secondary');
        if (s === 'idle') {
    ...
    byId('btn-stopwatch-secondary').addEventListener('click', () => {
        const s = status();
        if (s === 'running') {
            const previousSplit = laps.length > 0 ? laps[laps.length - 1].splitMs : 0;
    ...
    initChipGroup(byId('stopwatch-alert-interval'), {
        initial: '0',
        onChange: (value) => {
...
//# sourceMappingURL=stopwatch-view.js.map