...
export type Vo2maxProtocol = 'cooper' | 'rockport';

export interface Vo2maxEstimateResult {
  vo2max: number;
  protocol: Vo2maxProtocol;
}
...
export function estimateVo2maxCooper(distanceMeters: number): Vo2maxEstimateResult | null {
...
export function estimateVo2maxRockport(input: RockportInput): Vo2maxEstimateResult | null {
  const { weightKg, ageYears, sex, timeMinutes, heartRateBpm } = input;
  if (!(weightKg > 0) || !(ageYears > 0) || !(timeMinutes > 0) || !(heartRateBpm > 0)) return null;
