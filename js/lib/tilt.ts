...
export interface TiltHandle {
   ...
   * Call this from a click/tap handler on the surface itself; it's a
   * silent no-op on every other platform, where tilt already works
   * without asking. */
...
interface MaybeGatedDeviceOrientationEvent {
...
export function attachTilt(root: HTMLElement): TiltHandle {
  ...
  const visibilityObserver = new MutationObserver(() => {
    if (root.hidden) stopLoop();
    else startLoop();
  });
  visibilityObserver.observe(root, { attributes: true, attributeFilter: ['hidden'] });
  if (!root.hidden) startLoop();

  function onPointerMove(event: PointerEvent): void {
    if (event.pointerType === 'touch') return; // touch drives via device tilt instead
  ...
  const DOE = (window as unknown as { DeviceOrientationEvent?: MaybeGatedDeviceOrientationEvent })
