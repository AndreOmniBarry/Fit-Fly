...
export const TIME_RANGE_KEYS = ['D', 'W', 'M', '6M', 'Y'];
const RANGE_DAYS = { D: 1, W: 7, M: 30, '6M': 183, Y: 365 };
const RANGE_BUCKET = {
    D: 'day',
    W: 'day',
    M: 'day',
...
export function timeRangeBounds(range, todayDate) {
...
export function timeRangeDescription(range) {
    switch (range) {
        case 'D':
            return 'Today only.';
        case 'W':
...
export function bucketDailyPoints(points, bucket) {
    if (bucket === 'day') {
        return [...points]
...
export function formatBucketAxisLabel(key, bucket) {
    if (bucket === 'month') {
...
export function formatBucketDetailLabel(key, bucket) {
    if (bucket === 'month') {
