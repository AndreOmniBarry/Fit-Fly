...
export interface TrendChartPoint {
  /** A stable key for this point (e.g. an ISO date or a run id) — used
   *  only as a DOM key, never displayed. */
  key: string;
  /** The real value this bar's height represents. */
  value: number;
  /** Short text under the bar (e.g. a weekday initial) — kept sparse on
   *  purpose; the tooltip carries the exact value/date, see
   *  marks-and-anatomy's "label selectively" rule. */
  axisLabel: string;
  /** Drawn at full accent strength (a goal met, a personal best) instead
   *  of the dimmer default. */
  highlighted?: boolean;
  /** The bold first line of this bar's tooltip/aria-label — e.g. "6,234 steps". */
  tooltipValue: string;
  /** The muted second line — e.g. "Mon, Jun 2 · Goal met". */
  tooltipDetail: string;
}

export interface TrendChartOptions {
  points: TrendChartPoint[];
  /** A CSS custom property name (e.g. '--steps-accent') read from the
   *  container's own computed style — keeps this module themeable
   *  without hard-coding any mini-app's color into shared code. */
  accentVar: string;
  /** Draws a recessive reference line at this value (a goal, say) — the
   *  scale still auto-fits the real data even when every value clears
   *  or misses it, so the line moves relative to the bars, not the
   *  other way around. */
...
export function renderTrendChart(container: HTMLElement, options: TrendChartOptions): void {
  ...
  for (const point of points) {
    ...
    if (point.highlighted) fill.classList.add('trend-chart-bar-fill--highlighted');
    ...
    const showTooltip = () => {
      for (const other of bars.querySelectorAll<HTMLElement>('.trend-chart-tooltip')) other.hidden = true;
