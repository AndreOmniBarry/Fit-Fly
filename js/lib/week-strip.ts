...
export function buildWeekStrip(
  ...
  goalMetDates: ReadonlySet<string>,
  today: Date = new Date(),
  days = 7
): WeekStripDay[] {
  const todayIso = today.toISOString().slice(0, 10);
  const result: WeekStripDay[] = [];
...
export function renderWeekStrip(
  container: Element,
  days: WeekStripDay[],
  options: { icon: IconName; accentVar: string }
): void {
  clearChildren(container);
  container.classList.add('week-strip');
  (container as HTMLElement).style.setProperty('--week-strip-accent', `var(${options.accentVar})`);

  for (const day of days) {
    const cell = document.createElement('span');
    cell.className = 'week-strip-cell';
    if (day.goalMet) cell.classList.add('week-strip-cell--met');
    else if (day.hasEntry) cell.classList.add('week-strip-cell--logged');
    if (day.isToday) cell.classList.add('week-strip-cell--today');
    const dateLabel = new Date(`${day.date}T00:00:00`).toLocaleDateString(undefined, { weekday: 'short' });
    cell.setAttribute(
      'aria-label',
      `${dateLabel}: ${day.goalMet ? 'goal met' : day.hasEntry ? 'logged, goal not met' : 'nothing logged'}`
    );
